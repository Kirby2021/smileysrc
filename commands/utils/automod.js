    ...
    async run (bot, message, args) {
        ...
        if(!message.member.hasPermission('MANAGE_MESSAGES')) {
            if(!user || user.id === message.author.id) {
                ...
                var results = await punishmentSchemaAutoMod.find({
                ...
                let punishmentInformation = `All automod punishments for ${message.author}\n\n`;
                ...
                message.channel.send(punishments).catch(error => {
                    if(error.code == 50006) return message.lineReply(`You don't have active punishments.`)
                })
            } else if(user) {
                var results = await punishmentSchemaAutoMod.find({
                    guildID: message.guild.id,
            ...
            if(!user || user.id === message.author.id) {
                ...
                for(const punishmentObject of Array2) {
                    ...
                    punishmentInformation += `**ID: ${punishmentID} | Moderator:** ${bot.user.tag}\n**${punishmentType}** - ${reason} - ${time}\n\n`;
                ...
                let punishmentInformation = `All automod punishments for ${user}\n\n`;
                ...
                message.channel.send(punishments).catch(error => {
                    if(error.code == 50006) return message.lineReply(`That user doesn't have active punishments.`)
                })
            }
        }
    }
